...
def get_parser():
    ...
    parser.add_argument(
        "--split", type=str, default="test", help="Which split of the dataset to use"
    )
    ...
    parser.add_argument(
        "--layer", type=int, default=-1, help="Which layer to use (if not all layers)"
    )
    parser.add_argument(
        "--all_layers", action="store_true", help="Whether to use all layers or not"
    )
    parser.add_argument(
        "--token_idx",
        type=int,
        default=-1,
        help="Which token to use (by default the last token)",
    )
    # saving the hidden states
    parser.add_argument(
        "--save_dir",
        type=str,
        default="generated_hidden_states",
...
def load_model(model_name, cache_dir=None, parallelize=False, device="cuda"):
    ...
    try:
        model = AutoModelForSeq2SeqLM.from_pretrained(
            full_model_name, cache_dir=cache_dir
        )
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        full_model_name, cache_dir=cache_dir, model_max_length=512
    )
    model.eval()
...
def save_generations(generation, args, generation_type):
    ...
    filename = (
        ...
        + "__"
        + "__".join(
            ["{}_{}".format(k, v) for k, v in arg_dict.items() if k not in exclude_keys]
        )
...
class ContrastDataset(Dataset):
    ...
    def get_encoder_input_ids(self, question, answer):
        ...
        input_ids = self.tokenizer(
            combined_input, truncation=True, padding="max_length", return_tensors="pt"
        )
    ...
    def get_encoder_decoder_input_ids(self, question, answer):
        ...
        if self.use_decoder:
            ...
            input_ids = self.tokenizer(
                question, truncation=True, padding="max_length", return_tensors="pt"
            )
            decoder_input_ids = self.tokenizer(
                answer, truncation=True, padding="max_length", return_tensors="pt"
            )
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.use_decoder and self.model_type == "encoder_decoder":
            assert (
                neg_ids["decoder_input_ids"] - pos_ids["decoder_input_ids"]
            ).sum() != 0, print(
...
def get_individual_hidden_states(
    ...
    if all_layers:
        ...
        hs = torch.stack(
            [h.squeeze().detach().cpu() for h in hs_tuple], axis=-1
        )  # (bs, seq_len, dim, num_layers)
    else:
